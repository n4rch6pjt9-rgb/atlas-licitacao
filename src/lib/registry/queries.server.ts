...
import { claimFromEvidence } from "./format.ts";
import { buildFamilyCandidates, pickReuseFamily } from "./family-candidates.ts";
import { asEnum, asJsonObject, type JsonObject } from "./json.ts";
import type {
  AlertRecord,
  CensusRow,
  DiscoveryChannelRow,
...
import { applyPlatformTransition, currentPlatform, type HistoryTransition } from "./platform-history.ts";
...
function toJson(value: unknown): JsonObject | undefined {
...
function mapSource(row: Row): SourceListItem {
  return {
    id: String(row.id),
...
export async function listSources(sql: Sql, filter?: SourceListFilters): Promise<SourceListItem[]> {
  const clauses: string[] = [];
  const params: unknown[] = [];
  if (filter?.q) {
    params.push(`%${filter.q.toLowerCase()}%`);
    clauses.push(
      `(lower(s.name) like $${params.length} or lower(coalesce(s.base_url, '')) like $${params.length} or lower(s.slug) like $${params.length})`,
    );
  }
  if (filter?.uf) {
  ...
  return jsonSafe(
    (await sql.query<Row>(`${SOURCE_SELECT}${where} order by j.uf nulls first, s.name`, params)).map(mapSource),
  );
}

export async function getSourceById(sql: Sql, id: string): Promise<SourceListItem | null> {
  const row = (await sql.query<Row>(`${SOURCE_SELECT} where s.id = $1`, [id]))[0];
  return row ? jsonSafe(mapSource(row)) : null;
}

async function getRawSource(sql: Sql, id: string): Promise<Row | null> {
  return (await sql.query<Row>(`${SOURCE_SELECT} where s.id = $1`, [id]))[0] ?? null;
}

function capabilitySupportStatus(row: Row): string {
  const observed = bool(row.publicly_observed);
  const documented = bool(row.documented);
  const confidence = num(row.confidence);
  if (observed && documented && confidence >= 0.8) return "SUPPORTED";
  if (observed) return "PARTIAL";
  return "UNKNOWN";
}

function mapCapability(row: Row): JsonObject {
  const support_status = capabilitySupportStatus(row);
  const evidence =
    support_status === "SUPPORTED" ? "VERIFIED" : support_status === "PARTIAL" ? "WEAK_INDICATION" : "UNKNOWN";
  return asJsonObject({
    id: String(row.id),
    ...
    last_verified_at: iso(row.last_verified_at),
    claim_kind: claimFromEvidence(evidence as "VERIFIED" | "STRONG_INDICATION" | "WEAK_INDICATION" | "UNKNOWN"),
    raw_metadata: toJson(row.raw_metadata) ?? null,
  });
}

function mapEndpoint(row: Row): JsonObject {
  return asJsonObject({
    id: String(row.id),
    name: String(row.name ?? ""),
...
function mapEvidence(row: Row): JsonObject {
...
function mapObservation(row: Row): JsonObject {
  return asJsonObject({
    id: String(row.id),
...
function mapConnectorConfig(config: Row | undefined, source: Row): JsonObject | null {
...
export async function getSourceDetail(sql: Sql, id: string): Promise<SourceDetail | null> {
  const raw = await getRawSource(sql, id);
  if (!raw) return null;
  const list = mapSource(raw);
  const [capabilities, endpoints, evidence, observations, runs, history, configs, alerts, relationships, policies] =
    await Promise.all([
      sql.query<Row>(`select * from source_capability where source_system_id = $1 order by capability`, [id]),
      sql.query<Row>(`select * from source_endpoint where source_system_id = $1 order by path`, [id]),
      sql.query<Row>(`select * from source_evidence where source_system_id = $1 order by captured_at desc`, [id]),
      sql.query<Row>(`select * from source_observation where source_system_id = $1 order by observed_at desc`, [id]),
      sql.query<Row>(`select * from source_fingerprint_run where source_system_id = $1 order by started_at desc`, [id]),
      sql.query<Row>(`select * from source_platform_history where source_system_id = $1 order by valid_from desc`, [id]),
      sql.query<Row>(`select * from source_connector_config where source_system_id = $1`, [id]),
      sql.query<Row>(`select * from source_alert where source_system_id = $1 order by created_at desc`, [id]),
      sql.query<Row>(
        `select * from source_system_relationship
         where source_system_id = $1 or related_source_system_id = $1`,
        [id],
      ),
      sql.query<Row>(`select * from source_rate_policy where source_system_id = $1`, [id]),
    ]);
  const fingerprintRows: JsonObject[] = [];
  for (const run of runs) {
    const matches = await sql.query<Row>(
      `select m.*, sig.key as signature_key, sig.signature_type
       from source_fingerprint_match m
       left join fingerprint_signature sig on sig.id = m.signature_id
       where m.run_id = $1
       order by m.matched desc, m.score desc`,
      [run.id],
    );
    const rawResult = (run.raw_result ?? {}) as Record<string, unknown>;
    const classification = (rawResult.classification ?? {}) as Record<string, unknown>;
    const claim =
      classification.claim_kind === "FATO_VERIFICADO" ||
      classification.claim_kind === "INFERENCIA" ||
      classification.claim_kind === "PENDENTE_DE_VALIDACAO"
    ...
    fingerprintRows.push(
      asJsonObject({
        ...
        candidate_family: run.candidate_family
          ? asEnum(run.candidate_family, TECHNOLOGY_FAMILIES, "UNKNOWN")
          : null,
        technology_family:
          (classification.technology_family as string | undefined) ??
          (run.candidate_family ? asEnum(run.candidate_family, TECHNOLOGY_FAMILIES, "UNKNOWN") : null),
        ...
        evidence_level: (classification.evidence_level as string | null) ?? null,
        claim_kind: claim,
  ...
  const lastMatches = last && Array.isArray(last.matches) ? last.matches : [];
  const last_run_matches = lastMatches.map((match) => asJsonObject(match));
  const endpointIds = endpoints.map((row) => String(row.id));
  let healthRows: Row[] = [];
  if (endpointIds.length > 0) {
    const placeholders = endpointIds.map((_, i) => `$${i + 1}`).join(", ");
    healthRows = await sql.query<Row>(
      `select h.*, e.name as endpoint_name, e.path as endpoint_path
       from source_endpoint_health h
       left join source_endpoint e on e.id = h.source_endpoint_id
       where h.source_endpoint_id in (${placeholders})`,
      endpointIds,
    );
  }
  ...
  return jsonSafe({
    ...
    history: history.map((row) => {
      ...
      return asJsonObject({
        ...
        confidence: num(row.confidence),
        evidence_level: row.confidence != null && num(row.confidence) >= 0.8 ? "VERIFIED" : "UNKNOWN",
...
export async function listFamilies(sql: Sql): Promise<FamilySummary[]> {
  return jsonSafe(
    (
      await sql.query<Row>(`select
     ...
     order by source_count desc, family`)
      ...
      const adapters = String(row.connector_types ?? "")
        .split(",")
        .map((item) => item.trim())
        .filter(Boolean);
      ...
      return {
        ...
        census_pending:
          (VENDOR_FAMILIES.has(family) || family === "SISTEMA_PROPRIO") && Number(row.verified_count) === 0,
      };
    }),
  );
}

export async function capabilityMatrix(sql: Sql) {
  const rows = await sql.query<Row>(`select s.id as source_id, s.name as source_name, c.capability, c.publicly_observed
     from source_system s
     left join source_capability c on c.source_system_id = s.id
     order by s.name`);
  const bySource = new Map<
    string,
    { source_id: string; source_name: string; cells: Record<string, { present: boolean }> }
  >();
  for (const row of rows) {
    const sourceId = String(row.source_id);
    let entry = bySource.get(sourceId);
    if (!entry) {
      entry = { source_id: sourceId, source_name: String(row.source_name), cells: {} };
...
export async function metrics(sql: Sql): Promise<RegistryMetrics> {
  const [totals] = await sql.query<Row>(`select
     ...
     from source_system`);
  const byFamily = await sql.query<Row>(`select coalesce(technology_family, 'UNKNOWN') as family, count(*)::int as count
     from source_system
     group by coalesce(technology_family, 'UNKNOWN')
     order by count desc, family`);
  const [reuse] = await sql.query<Row>(`with typed as (
...
export async function listJurisdictions(sql: Sql): Promise<JurisdictionRecord[]> {
  return jsonSafe(
    (await sql.query<Row>(`select * from jurisdiction where active = true order by type, uf nulls first, name`)).map(
      (row) => ({
        id: String(row.id),
...
export async function listAlerts(
  sql: Sql,
  filter?: { sourceId?: string; unresolvedOnly?: boolean },
): Promise<AlertRecord[]> {
  const clauses: string[] = [];
  const params: unknown[] = [];
  if (filter?.sourceId) {
...
export async function listSignatures(sql: Sql): Promise<FingerprintSignature[]> {
  return (await sql.query<Row>(`select * from fingerprint_signature`)).map((row) => ({
    id: String(row.id),
    technology_family: row.technology_family as FingerprintSignature["technology_family"],
    signature_type: row.signature_type as FingerprintSignature["signature_type"],
    key: String(row.key),
...
export async function listObservationsForSource(sql: Sql, sourceId: string): Promise<Observation[]> {
  return (
    await sql.query<Row>(
      `select observation_type, key, value, source_url, raw_payload
     from source_observation
     where source_system_id = $1`,
      [sourceId],
    )
  ).map((row) => ({
    observation_type: row.observation_type as Observation["observation_type"],
    key: String(row.key),
    value: String(row.value),
    source_url: row.source_url == null ? null : String(row.source_url),
  ...
  for (const obs of observations) {
    ...
    await sql.query(
      ...
      [
        ...
        observedAt,
        obs.source_url ?? null,
        obs.raw_payload === undefined ? null : JSON.stringify(obs.raw_payload),
      ],
    );
    ids.push(id);
  }
...
export async function insertFingerprintRun(
  sql: Sql,
  input: {
    id: string;
  ...
  await sql.query(
    `insert into source_fingerprint_run (
    ...
    [
      ...
      input.raw_result === undefined ? null : JSON.stringify(input.raw_result),
      input.error ?? null,
    ],
  );
  if (input.matches) {
    for (const match of input.matches) {
      ...
      await sql.query(
        `insert into source_fingerprint_match (
...
export async function insertAlert(
  ...
  await sql.query(
    `insert into source_alert (id, source_system_id, alert_type, severity, message, payload)
...
export async function insertSourceRecord(
  ...
  const publicUrl =
    pncpEditalUrl(input.source_identifier) ??
    toPublicPncpUrl(input.source_url) ??
    (input.source_system_id === "src_br_pncp" ? null : input.source_url ?? null);
...
export async function upsertSourceCapability(
  sql: Sql,
  input: {
    id?: string;
    source_system_id: string;
...
async function loadPlatformHistory(sql: Sql, sourceId: string): Promise<PlatformHistoryEntry[]> {
  return (
    await sql.query<Row>(`select * from source_platform_history where source_system_id = $1 order by valid_from`, [
      sourceId,
    ])
  ).map((row) => ({
    id: String(row.id),
    source_system_id: String(row.source_system_id),
    technology_family: row.technology_family as TechnologyFamily,
    vendor_name: row.vendor_name == null ? null : String(row.vendor_name),
    product_name: row.product_name == null ? null : String(row.product_name),
    valid_from: iso(row.valid_from) ?? new Date().toISOString(),
    valid_to: iso(row.valid_to),
...
async function persistPlatformHistory(
  sql: Sql,
  sourceId: string,
  ...
  if (result.closed) {
    await sql.query(`update source_platform_history set valid_to = $2 where id = $1`, [
      result.closed.id,
      result.closed.valid_to,
    ]);
  }
  if (result.created) {
    const created = result.created;
    await sql.query(
      `insert into source_platform_history (
...
function nextClassificationState(current: string | null, result: ClassificationResult): ClassificationState {
...
export async function applyClassificationToSource(sql: Sql, sourceId: string, result: ClassificationResult) {
  ...
  if (result.conflict && officialVendor) {
    ...
    if (officialVendor && result.vendor_name && result.vendor_name !== nextVendor) {
      await insertAlert(sql, {
        source_system_id: sourceId,
        alert_type: "SOURCE_VENDOR_CONFLICT",
        severity: "high",
        message: `Fornecedor oficial ${raw.vendor_name} vs fingerprint ${result.vendor_name}`,
        payload: { official: raw.vendor_name, fingerprint: result.vendor_name },
      });
    } else {
    ...
    if (!VENDOR_FAMILIES.has(currentFamily) || String(raw.vendor_evidence_level) !== "VERIFIED") {
      nextFamily = classifiedFamily;
    }
    if (!officialVendor) {
      nextVendor = null;
  ...
  const state = nextClassificationState(
    raw.classification_state == null ? null : String(raw.classification_state),
    result,
  );
  await sql.query(
    `update source_system set
       ...
       updated_at = now()
     where id = $1`,
    [sourceId, nextFamily, nextVendor, nextProduct, nextLevel, nextConfidence, state],
  );
  const open = currentPlatform(await loadPlatformHistory(sql, sourceId), sourceId);
  if (
    (!open ||
      open.technology_family !== nextFamily ||
      (open.vendor_name ?? null) !== (nextVendor ?? null) ||
      (open.product_name ?? null) !== (nextProduct ?? null)) &&
    nextFamily !== "UNKNOWN"
  ) {
    await persistPlatformHistory(sql, sourceId, {
      source_system_id: sourceId,
  ...
  if (result.evidence_level === "VERIFIED" && classifiedFamily !== "UNKNOWN" && classifiedFamily !== currentFamily) {
    await insertAlert(sql, {
      source_system_id: sourceId,
      alert_type: "SOURCE_FAMILY_CONFIRMED",
      severity: "info",
      message: `Família confirmada: ${classifiedFamily}`,
      payload: { family: classifiedFamily, previous: currentFamily },
    });
  }
  return {
...
export async function insertRegisteredSource(
  sql: Sql,
  input: { name: string; baseUrl: string; jurisdictionId: string; functionalFamily?: string },
  ...
  await sql.query(
    ...
    [id, input.jurisdictionId, input.name, slug, input.baseUrl, input.functionalFamily ?? "UNKNOWN", "Registrado sem classificação de fornecedor."],
  );
  await sql.query(
    `insert into source_rate_policy (
...
export async function loadEvidenceInputs(sql: Sql, sourceId: string): Promise<EvidenceInput[]> {
  return (await sql.query<Row>(`select * from source_evidence where source_system_id = $1`, [sourceId])).map((row) => ({
    evidence_type: row.evidence_type as EvidenceInput["evidence_type"],
    evidence_level: (row.evidence_level as EvidenceInput["evidence_level"]) ?? undefined,
    title: String(row.title ?? ""),
...
export async function loadConnectorConfig(sql: Sql, sourceId: string): Promise<ConnectorConfig | null> {
  const row = (
    await sql.query<Row>(
      `select c.*, s.base_url as source_base_url, s.connector_type as source_connector_type
     from source_system s
  ...
  if (!row) return null;
  const connectorType = asEnum(row.connector_type ?? row.source_connector_type, CONNECTOR_TYPES, "NONE") as ConnectorType;
  ...
  const pathsJson = row.paths_json as Record<string, unknown> | null;
  const paths = asStringMap(row.paths_json);
  const responsePath = typeof pathsJson?.responsePath === "string" ? String(pathsJson.responsePath) : undefined;
  const linkBase = typeof pathsJson?.linkBase === "string" ? String(pathsJson.linkBase) : undefined;
  return {
    connectorType,
    ...
    responsePath,
    pagination: (row.pagination_config as ConnectorConfig["pagination"]) ?? undefined,
...
export async function listFamilyCandidates(sql: Sql): Promise<FamilyCandidateRow[]> {
  const sources = await listSources(sql);
  return jsonSafe(buildFamilyCandidates(sources));
}

export async function listCensusRows(sql: Sql): Promise<CensusRow[]> {
  const sources = await listSources(sql);
  const history = await sql.query<Row>(`select source_system_id, technology_family, vendor_name, product_name, valid_from, valid_to
    ...
    sources.map((source) => {
      ...
      const period =
        closed.length > 0
          ? [
              ...closed.map((row) =>
                `${iso(row.valid_from)?.slice(0, 4) ?? "?"}–${iso(row.valid_to)?.slice(0, 4) ?? "?"} ${row.technology_family ?? ""}`.trim(),
              ),
              current
                ? `${iso(current.valid_from)?.slice(0, 4) ?? "?"}– ${source.technology_family ?? "UNKNOWN"}`
                : `vigente ${source.technology_family ?? "UNKNOWN"}`,
            ].join(" · ")
          : `vigente ${source.technology_family ?? "UNKNOWN"}`;
...
export async function milestone3Metrics(sql: Sql): Promise<Milestone3Metrics> {
  const sources = await listSources(sql);
  const candidates = buildFamilyCandidates(sources);
  const base = await metrics(sql);
  const perAdapter = await sql.query<Row>(`select coalesce(nullif(connector_type, ''), 'NONE') as adapter, count(*)::int as count
     from source_system
     group by coalesce(nullif(connector_type, ''), 'NONE')
     order by count desc`);
  const distribution = await sql.query<Row>(`select coalesce(vendor_evidence_level, 'UNKNOWN') as level, count(*)::int as count
     from source_system
     group by coalesce(vendor_evidence_level, 'UNKNOWN')
     order by count desc`);
  const [overlap] = await sql.query<Row>(`select
       count(*) filter (where pncp_overlap = true)::int as n,
       count(*)::int as total
     from source_system`);
  const [localOnly] = await sql.query<Row>(`select count(*)::int as n from source_record r
     left join source_entity_link l
       on l.source_system_id = r.source_system_id
      and l.source_external_id = r.source_identifier
     where l.id is null`);
  const [historyChanges] = await sql.query<Row>(
    `select count(*)::int as n from source_platform_history where valid_to is not null`,
  );
  const pair = compareSchemas(
    fingerprintPayload("src_sc_compras", {
      conteudo: [
        {
          id: 1,
          processo: "1/2026",
    ...
    fingerprintPayload("src_br_pncp", {
      data: [{ numeroControlePNCP: "x", numeroCompra: "1", orgaoEntidade: { razaoSocial: "y" }, objetoCompra: "z" }],
    }),
  );
...
function mapDiscoveryChannel(row: Row): DiscoveryChannelRow {
  return {
    id: String(row.id),
    source_system_id: String(row.source_system_id ?? ""),
...
export async function listDiscoveryChannels(
  sql: Sql,
  filters: { sourceId?: string; family?: string } = {},
): Promise<DiscoveryChannelRow[]> {
  try {
    const clauses: string[] = [];
    const params: unknown[] = [];
    if (filters.sourceId) {
    ...
    const rows = await sql.query<Row>(
      `select c.* from source_discovery_channel c ${where}
    ...
    const mapped = rows.map(mapDiscoveryChannel);
