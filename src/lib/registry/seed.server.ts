...
import { FRAMEWORK_SIGNATURES } from "./signatures";
import { PNCP_OFFICIAL_NAME, pncpEditalUrl, toPublicPncpUrl } from "./pncp-url.ts";
...
async function upsert(sql: Sql, table: string, row: Record<string, unknown>, pk = "id"): Promise<void> {
...
async function rewriteUrlColumn(
	sql: Sql,
	table: string,
	column: string,
): Promise<void> {
	const rows = await sql.query<{ id: string; url: string | null }>(
...
async function repairPncpPublicUrls(sql: Sql): Promise<void> {
	await sql.query(
     ...
     set name = $1,
         ...
         base_url = 'https://pncp.gov.br',
         notes = $2
     where id = 'src_br_pncp'`,
		[
			PNCP_OFFICIAL_NAME,
			"FATO — Portal Nacional de Contratações Públicas. URL pública de edital: https://pncp.gov.br/app/editais/{cnpj}-{unidade}-{numero}/{ano} (ex.: https://pncp.gov.br/app/editais/75442756000190-1-001001/2026). API de consulta em /api/consulta. Discover: /api/consulta/v1/contratacoes/publicacao.",
		],
	);
	...
	const records = await sql.query<{
		...
		`select id, source_identifier, source_url from source_record
     where source_system_id = 'src_br_pncp'`,
	);
	for (const rec of records) {
		const next =
			pncpEditalUrl(rec.source_identifier) ?? toPublicPncpUrl(rec.source_url);
		if (next !== rec.source_url) {
			await sql.query(`update source_record set source_url = $1 where id = $2`, [
				next,
				rec.id,
			]);
		}
	...
	await upsert(sql, "source_discovery_channel", {
		...
		data_origin: "GOLDEN",
		notes:
			"Padrão oficial do Portal Nacional de Contratações Públicas. Ex.: https://pncp.gov.br/app/editais/75442756000190-1-001001/2026. Não usar /pncp-api/ como URL pública.",
	});
}

async function repairExistingSeeds(sql: Sql): Promise<void> {
...
async function seedAll(sql: Sql): Promise<void> {
	const already = ((await sql.query<{ n: number }>("select count(*)::int as n from jurisdiction"))[0]?.n ?? 0) > 0;
	...
	if (already) {
		if (((await sql.query<{ n: number }>("select count(*)::int as n from source_system"))[0]?.n ?? 0) > 0) {
			await repairExistingSeeds(sql);
			const { seedMilestone3 } = await import("./seed-m3.server");
			await seedMilestone3(sql);
			const { seedMilestone4 } = await import("./seed-m4.server");
			await seedMilestone4(sql);
			const { seedMilestone5 } = await import("./seed-m5.server");
			await seedMilestone5(sql);
			const { seedMilestone6 } = await import("./seed-m6.server");
			await seedMilestone6(sql);
			const { seedMilestone7 } = await import("./seed-m7.server");
			await seedMilestone7(sql);
			const { seedGate75 } = await import("./seed-g75.server");
			await seedGate75(sql);
			const { seedBllChannels } = await import("./seed-bll-channels.server");
...
const globalRef = globalThis as typeof globalThis & {
  __atlasRegistrySeedG75j__?: Promise<void>;
};
export function ensureSeeded(sql: Sql): Promise<void> {
	globalRef.__atlasRegistrySeedG75j__ ??= seedAll(sql).catch((err) => {
		globalRef.__atlasRegistrySeedG75j__ = undefined;
		console.error("[atlas-seed]", err);
		throw err;
	});
