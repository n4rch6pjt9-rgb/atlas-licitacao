...
export type PublicProcurementSource = {
  ...
  discover(params: DiscoveryParams): AsyncIterable<SourceProcurement>;
  getPlanning(params: DiscoveryParams): Promise<SourcePlanningRecord[]>;
};

export type HtmlListRecord = {
  external_id: string;
...
export function mapValue(obj: unknown, path: string): unknown {
...
export function applyNormalization(
  ...
  const paths = { ...DEFAULT_FIELD_PATHS, ...(mapping ?? {}) };
  ...
  const control =
    asString(mapValue(raw, "numeroControlePNCP")) ??
    asString(mapValue(raw, "numeroControlePncp")) ??
    mappedString(raw, paths, "external_id", ["numeroControlePNCP"]);
  const external_id =
    mappedString(raw, paths, "external_id", [
      "id",
      "codigo",
      "uuid",
      "numeroControlePNCP",
    ...
    "unknown";
  const year =
    asYear(paths.year ? mapValue(raw, paths.year) : undefined) ??
    asYear(mapValue(raw, "ano")) ??
    asYear(mapValue(raw, "anoCompra")) ??
    asYear(mapValue(raw, "year")) ??
  ...
  return {
    ...
    source_system_id,
    process_number,
    procurement_number: mappedString(raw, paths, "procurement_number", [
      "numero",
      "numeroCompra",
    ]),
    year,
    ...
    organization_identifier: mappedString(raw, paths, "organization_identifier", [
      "orgaoCnpj",
      "cnpj",
    ]),
    ...
    proposal_deadline: mappedString(raw, paths, "proposal_deadline", [
      "entregaProposta",
    ]),
...
function decodeEntities(text: string): string {
  return text
    ...
    .replace(/&#x([0-9a-f]+);/gi, (_, n) =>
      String.fromCharCode(parseInt(String(n), 16)),
    );
}
...
export function idFromHref(href: string): string | null {
  try {
    ...
    if (last && /View$|Search$|\.action$/i.test(last) && url.search) {
      return url.search.replace(/^\?/, "") || last;
    }
    return last ?? null;
  } catch {
    return null;
...
const PROCESS_NUMBER_RE =
  /^\d{1,8}(?:[./-]\d{1,8}){0,3}(?:[./-][A-Za-z0-9]{1,8})?$/;

function looksLikeProcessNumber(cell: string): boolean {
...
function columnRole(
  ...
  if (/(promotor|orgao|organizacao|organization|ente|municipio)/.test(folded)) {
...
function fallbackModality(cell: string): string | null {
  const text = cell.trim();
  if (
    /^(dispensa|inexigibilidade|pregao|preg[aã]o|concorrencia|concorr[eê]ncia|tomada|leilao|leil[aã]o|dialogo|credenciamento)/i.test(
      text,
    )
  ) {
    return text;
...
function fallbackStatus(cell: string): string | null {
  const text = cell.trim();
  if (
    /^(publicada|conclu[ií]da|aberta|encerrada|suspensa|anulada|fracassada|deserta|em andamento)/i.test(
      text,
    )
  ) {
    return text;
...
function recordFromRow(input: {
  ...
  const process_number =
    mapped.process_number ??
    cells.find((cell) => looksLikeProcessNumber(cell)) ??
    null;
  const organization_name = mapped.organization_name ?? null;
  const modality =
    mapped.modality ??
    cells.map(fallbackModality).find((value) => Boolean(value)) ??
    null;
  const status =
    mapped.status ??
    cells.map(fallbackStatus).find((value) => Boolean(value)) ??
    null;
  const opening_at = mapped.opening_at ?? null;
  let object = mapped.object ?? null;
  if (!object) {
    const skip = new Set(
      [process_number, organization_name, modality, status, opening_at].filter(
        Boolean,
      ) as string[],
    );
    object =
  ...
  return {
    external_id:
      (href ? idFromHref(href) : null) ??
      process_number ??
      source_url ??
      stripTags(input.rowHtml).slice(0, 80) ??
      "unknown",
    process_number,
    object,
    source_url,
    htmlSnippet: input.fullRow.slice(0, 4000),
    organization_name,
    modality,
    status,
    opening_at,
...
export function parseHtmlListRecords(
  html: string,
  baseUrl: string,
  linkPattern: RegExp,
): HtmlListRecord[] {
  const records: HtmlListRecord[] = [];
  ...
  for (const headerMatch of html.matchAll(
    /<tr\b[^>]*>((?:\s*<th\b[\s\S]*?<\/th>\s*)+)<\/tr>/gi,
  )) {
    const ths = [...(headerMatch[1] ?? "").matchAll(/<th\b[^>]*>([\s\S]*?)<\/th>/gi)].map(
  ...
  for (const rowMatch of html.matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/gi)) {
    ...
    if (
      [...rowHtml.matchAll(/<th\b[^>]*>([\s\S]*?)<\/th>/gi)].map((match) =>
        stripTags(match[1] ?? ""),
      ).length > 0 &&
      !/<td\b/i.test(rowHtml)
    ) {
    ...
    const hrefs = [...rowHtml.matchAll(/href=["']([^"']+)["']/gi)]
      .map((match) => match[1])
      .filter((href): href is string => Boolean(href && hrefMatches(href)));
    if (hrefs.length === 0) continue;
    if (/<(?:input|select|textarea)\b/i.test(rowHtml)) continue;
    const rawCells = [...rowHtml.matchAll(/<td\b[^>]*>([\s\S]*?)<\/td>/gi)].map(
      (match) => stripTags(match[1] ?? ""),
    );
  ...
  for (const match of html.matchAll(
    ...
    const processMatch = looksLikeProcessNumber(text)
      ? text
      : (text.match(/\d+\s*\/\s*\d{2,4}/)?.[0] ?? null);
    records.push({
      external_id: idFromHref(href) ?? (text || source_url),
      process_number: processMatch,
      object: text || null,
      source_url,
      htmlSnippet: (match[0] ?? "").slice(0, 4000),
...
export function htmlRecordToProcurement(
  row: HtmlListRecord,
  sourceSystemId: string,
  sourceChannel?: DiscoveryChannelType | null,
): SourceProcurement {
  return {
