...
/**
 * Product `src/lib/og/site.json` must not leak into platform OG tests.
 * Passing `site: {}` keeps resolveOgTitle on document title / appName / host.
 */
function isolateOg(ctx = {}) {
  return ctx.site !== undefined ? ctx : { ...ctx, site: {} };
}

function injectGrokPwaHead(html, ctx) {
  return injectGrokPwaHeadImpl(html, isolateOg(ctx));
}

function createHeadInjector(ctx) {
  return createHeadInjectorImpl(isolateOg(ctx));
}
