...
test("the auth schema ships outside the globbed directory", () => {
  ...
  assert.ok(
    ...
    "0001_auth.sql must stay in migrations/auth/, not the globbed directory",
  );
  assert.ok(readdirSync(join(migrationsDir, "auth")).includes(AUTH_MIGRATION));
  const pending = pendingMigrations(entries, []);
  assert.ok(
    pending.some((row) => row.name === "0002_portal_registry.sql"),
    "portal registry SQL must be globbed from migrations/",
  );
  assert.equal(
    pending.some((row) => row.name === AUTH_MIGRATION),
    false,
  );
});
