...
import { a as leadTimeHours, n as classifyLocalOnly, o as nextRecheckAt } from "./coverage-h_nbs-Bf.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/seed-m4.server-BsBmWg3n.js
...
var NOW = /* @__PURE__ */ new Date("2026-09-14T20:00:00.000Z");
...
var BLL_ENTES = [
	...
	},
	{
		...
		excerpt: "plataforma eletrônica de Bolsa de Licitações e Leilões do Brasil - BLL, aba Pregão Eletrônico, disponível no endereço eletrônico bllcompras.com"
	},
	{
	...
	},
	{
		...
		excerpt: "plataforma da BLL - Bolsa de Licitações e Leilões do Brasil Ltda. CADASTRO: http://www.bll.org.br"
	},
	{
	...
	},
	{
		...
		excerpt: "sistema BLL: https://bllcompras.com/Home/PublicAccess — Município de MORADA NOVA/CE, CNPJ 02.135.340/0001-55."
	},
	{
		...
		excerpt: "sessão pública por meio de sistema eletrônico - BLL - Bolsa de Licitações do Brasil, https://bllcompras.com/Home/Login."
	},
	{
	...
	},
	{
		...
		excerpt: "plataforma eletrônica bllcompras.com — Prefeitura Municipal de Juazeiro do Norte, CNPJ 07.974.082/0001-14."
	},
	{
	...
	},
	{
		...
		excerpt: "credenciamento no endereço eletrônico https://bllcompras.com — Prefeitura Municipal de Pedro de Toledo."
	},
	{
		...
		excerpt: "Portal Bolsa de Licitações e Leilões do Brasil-BLL, no sítio eletrônico https://bllcompras.com/Home/Login."
	},
	{
		id: "src_bll_irati_pr",
		jur: "jur_irati_pr",
		...
		excerpt: "PLATAFORMA DE DISPUTA: BLL. O Município de Irati, CNPJ 75.654.574/0001-82, www.bll.org.br."
	},
	{
		...
		excerpt: "PLATAFORMA: WWW.BLLCOMPRAS.ORG.BR — Bolsa de Licitações e Leilões - BLL. Município de BORBOREMA/SP."
	},
	{
		...
		excerpt: "Plataforma BLL Compras constante da página eletrônica da Bolsa de Licitações e Leilões do Brasil (bllcompras.com)."
	},
	{
	...
	},
	{
		...
		excerpt: "provedor bllcompras.com. Edital de 2022 — não prova sozinho que o portal vigente em 2026 continua BLL."
	}
];
var GAPS = [
	{
...
function asProcurement(ente, seq, extra) {
	const process = `${String(seq).padStart(3, "0")}/2026`;
	const object = typeof extra.objeto === "string" ? extra.objeto : `Aquisição de materiais e serviços — ${ente.name} ${process}`;
	return {
		...
		object,
		proposal_deadline: extra.deadline ?? "2026-09-20T17:00:00.000Z",
		opening_at: extra.opening ?? "2026-09-21T10:00:00.000Z",
...
async function seedMilestone4(sql) {
	for (const row of PARENTS) await upsert(sql, "jurisdiction", {
		id: row.id,
		type: "STATE",
		ibge_code: row.ibge,
		uf: row.uf,
	...
	for (const ente of BLL_ENTES) await upsert(sql, "jurisdiction", {
		id: ente.jur,
		type: ente.jurType,
		ibge_code: ente.ibge,
		uf: ente.uf,
	...
	for (const gap of GAPS) await upsert(sql, "jurisdiction", {
		id: gap.id,
		type: "MUNICIPALITY",
		ibge_code: gap.ibge,
		uf: gap.uf,
	...
	for (const ente of BLL_ENTES) {
		await upsert(sql, "source_system", {
			...
			vendor_confidence: ente.level === "VERIFIED" ? 1 : .75,
		...
		await upsert(sql, "source_platform_history", {
			...
			confidence: ente.level === "VERIFIED" ? 1 : .75
		});
	}
           ...
           do nothing`, [
			...
			if (slot === 0) {
             ...
             do nothing`, [
				...
				if (openingLocal !== openingPncp) await upsert(sql, "source_comparison", {
	...
	for (const ente of BLL_ENTES) await upsert(sql, "municipality_census", {
		id: `mc_${ente.jur}`,
		jurisdiction_id: ente.jur,
		ibge_code: ente.ibge,
		uf: ente.uf,
	...
	for (const gap of GAPS) await upsert(sql, "municipality_census", {
		id: `mc_${gap.id}`,
	...
	await upsert(sql, "source_live_probe", {
		...
		excerpt: "title SCPI - Licitações; /comprasedital/comprasedital.dll; css/fiorilli.css; ExtJS 7.9 SPA. Sem tabela HTML pública nem JSON de editais no GET inicial.",
		error: null,
		probed_at: OBSERVED
	});
	await upsert(sql, "source_live_probe", {
	...
	await upsert(sql, "source_live_probe", {
		...
		ok: true,
		content_type: "text/html; charset=utf-8",
		excerpt: "XHTML ASP.NET ctl00_Head1; /portal/css/portalcss; kendoUI metro. Não é JSF. Contrato JSON de editais não extraído.",
		error: null,
		probed_at: OBSERVED
	});
	await upsert(sql, "source_live_probe", {
		...
		ok: true,
		content_type: "text/html; charset=utf-8",
		excerpt: "Mesmos assets de Florianópolis: /portal/css/portalcss, uniform.claren.css, template2.css, kendoUI. Família técnica compartilhada, 2 fontes — escala insuficiente.",
		error: null,
		probed_at: OBSERVED
	});
	...
	await upsert(sql, "source_alert", {
		...
		severity: "info",
		message: "M4: 20 entes BLL, 1 generic-action, 120 registros locais ingeridos. Overlap passa a ser de records, não de declaração.",
		payload: {
			family: "BLL",
			entes: BLL_ENTES.length,
			records: 120
		}
	});
	await upsert(sql, "source_alert", {
		...
		severity: "info",
		message: "Assis ao vivo: SCPI ExtJS (comprasedital.dll + fiorilli.css). Sem lista HTML/JSON pública. FiorilliAdapter = NO-GO. Itapira timeout.",
		payload: {
	...
	await upsert(sql, "source_alert", {
		...
		alert_type: "SOURCE_FAMILY_CONFIRMED",
		severity: "info",
		message: "Florianópolis e Barueri ao vivo compartilham /portal/css/portalcss e kendoUI (ASP.NET, não JSF). 2 fontes — escala insuficiente para adapter.",
		payload: {
...
BLL_ENTES.map((row) => row.id);
BLL_ENTES.map((row) => row.org);
//#endregion
export { seedMilestone4 };
