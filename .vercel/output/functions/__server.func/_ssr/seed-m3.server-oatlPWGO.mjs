import { a as toPublicPncpUrl } from "./pncp-url-CkM9bQPG.mjs";
import { n as sha256 } from "./hash-DAnDaBOp.mjs";
import { t as LEARNED_VENDOR_SIGNATURES } from "./seed.server-DIgxylBp.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/seed-m3.server-oatlPWGO.js
...
var TRANSFEREGOV = "https://www.gov.br/transferegov/pt-br/sobre/apis-integracao/sistemas-de-compras";
...
var OBSERVED_AT = "2026-09-14T00:00:00.000Z";
function withEvidenceProvenance(row) {
	const url = typeof row.url === "string" ? toPublicPncpUrl(row.url) : row.url ?? null;
	const excerpt = typeof row.raw_excerpt === "string" && row.raw_excerpt || typeof row.description === "string" && row.description || typeof row.observed_value === "string" && row.observed_value || "";
	return {
		...row,
		url,
		observed_at: row.observed_at ?? OBSERVED_AT,
		content_hash: row.content_hash ?? sha256([
			row.id,
			row.title,
			url,
			row.publisher,
...
var BLL_DISCOVER = "/Process/ProcessSearchPublic?param1=0";
var BLL_LINK = "Process/ProcessView";
var SOURCES = [
	{
		id: "src_bll_platform",
	...
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: true,
		notes: "FATO — Edital Dispensa Eletrônica 02/2026 do Município de Cambará/PR publica a sessão na plataforma BLL (https://bllcompras.com/Home/Login). Mesmo portal e mesmo contrato HTML da plataforma nacional. Config filtra o órgão; o adapter não conhece o município."
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: true,
		notes: "FATO — Pregão Eletrônico 027/2025 do Município de Nova Olímpia/MT designa a Bolsa de Licitações e Leilões do Brasil e cita integração ao PNCP. Edital disponível em bll.org.br e no site da prefeitura."
	},
	{
		...
		vendor_confidence: .75,
		connector_type: "GENERIC_ACTION",
		classification_state: "ADAPTER_READY",
		discovery_strategy: "HTML_LIST",
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: true,
		notes: "INFERÊNCIA — MUNICIPIO DE ASPASIA aparece como promotor na busca pública BLL em 2026-09-14. Não há edital municipal arquivado neste censo. Nível STRONG_INDICATION, não VERIFIED."
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: true,
		notes: "INFERÊNCIA — MUNICIPIO DE BATURITE listado como promotor na busca pública BLL em 2026-09-14. Sem edital municipal neste censo."
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: true,
		notes: "INFERÊNCIA — MUNICIPIO DE NOVA ANDRADINA listado como promotor na busca pública BLL em 2026-09-14. Sem edital municipal neste censo."
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: true,
		notes: "FATO — Pregão Eletrônico 003/2026 do Município de Ubaíra/BA (CNPJ 13.910.690/0001-68) designa a Bolsa de Licitações e Leilões do Brasil em https://bllcompras.com/Home/Login. Mesmo portal nacional. Config filtra o órgão."
	},
	{
		id: "src_bll_cotia_sp",
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: true,
		notes: "FATO — Pregão Eletrônico SRP 013/2026 da Prefeitura de Cotia/SP realiza a sessão na plataforma BLL (bll.org.br / bllcompras.com). Período vigente 2026."
	},
	{
		id: "src_pcp_platform",
		jurisdiction_id: "jur_br",
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: true,
		notes: "FATO — Transferegov: PORTAL DE COMPRAS PÚBLICAS / ECUSTOMIZE CONSULTORIA EM SOFTWARE LTDA, CNPJ 09.397.355/0001-30. Homepage Angular/Express observada. Busca /processos existe. API JSON pública de editais NÃO extraída nesta sessão — adapter genérico PENDENTE."
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: false,
		notes: "FATO — Contrato 5/2024 da Câmara Municipal de Linhares: contratação do sistema Portal de Compras Públicas, contratada ECUSTOMIZE CONSULTORIA EM SOFTWARE S.A, CNPJ 09.397.355/0001-30."
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: false,
		notes: "FATO — Site oficial da Prefeitura de Alegre/ES declara adesão ao Portal de Compras Públicas, mantido pela Ecustomize Consultoria em Software S/A."
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: false,
		notes: "FATO — Edital PNCP do Município de Itapira/SP cita SCPI – Portal de Compras – Pregão Eletrônico – FIORILLI em http://transparencia.itapira.sp.gov.br:8079/comprasedital/. Instalação própria por ente. Adapter reutilizável PENDENTE de contrato técnico compartilhado ao vivo."
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: false,
		notes: "FATO — Publicação oficial do Município de Irapuã/SP cita SCPI – Portal de Compras – Pregão Eletrônico FIORILLI em http://170.0.49.246:5656/comprasedital/. Path /comprasedital compartilhado com Itapira. Fingerprint ao vivo PENDENTE."
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: false,
		notes: "FATO — Diário oficial do Município de Urupês/SP (leilão eletrônico 2025) nomeia SCPI – PORTAL DE COMPRAS (FIORILLI) em http://transparencia.urupes.sp.gov.br:5656/comprasedital/. Manual do fornecedor em ajuda.fiorilli.com.br. Instalação própria. Contrato HTML ao vivo PENDENTE."
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: false,
		notes: "FATO — Editais 2025 da Prefeitura de Assis/SP (CNPJ 46.179.941/0001-35) publicam a sessão no Portal de Compras Fiorilli https://scpi.assis.sp.gov.br:8079/comprasedital/. Host scpi.{municipio} é instalação, não prova sozinho; o edital nomeia Fiorilli."
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: false,
		notes: "FATO — Concorrência eletrônica 002/2025 da Prefeitura Municipal de Sales/SP (CNPJ 46.613.196/0001-90) nomeia SCPI – Portal de Compras – Fiorilli em http://170.0.48.169:8079/comprasedital/. Path compartilhado. IP não é evidência de vendor; o edital é."
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: false,
		notes: "FATO — Portal oficial da Prefeitura de Florianópolis em wbc.pmf.sc.gov.br identifica-se como 'Paradigma EGOV - florianopolis' (observado 2026-09-14). Produto vigente. Adapter reutilizável PENDENTE — HTML público, contrato JSON não extraído."
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: false,
		notes: "FATO — Portal oficial compras.barueri.sp.gov.br publica manual de cadastro WBC (janeiro/2025) com marca paradigmabs.com.br. Produto vigente. Contrato técnico JSON PENDENTE."
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: false,
		notes: "FATO — Transferegov: LICITANET, CNPJ 21.280.462/0001-80. GET público retornou HTTP 403 nesta sessão. Contrato técnico PENDENTE."
	},
	{
		id: "src_bnc_platform",
		jurisdiction_id: "jur_br",
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: false,
		notes: "FATO — Transferegov: BNC COMPRAS / Bolsa Nacional de Compras, CNPJ 25.099.967/0001-01. Site institucional WordPress observado. Portal transacional PENDENTE."
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: false,
		notes: "FATO — Transferegov: BBMNET Licitações / Bolsa Brasileira de Mercadorias, CNPJ 05.342.088/0001-43. Homepage institucional observada. URL transacional PENDENTE."
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: false,
		notes: "FATO — Transferegov: LICITAR DIGITAL SERVIÇOS EM TI LTDA, CNPJ 35.125.567/0001-79. Homepage bloqueada por Cloudflare nesta sessão. Fingerprint PENDENTE."
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: false,
		notes: "FATO — Transferegov: PREGÃO ELETRÔNICO SIGA / COMPRASBR / A Z INFORMÁTICA LTDA, CNPJ 24.598.492/0001-27. NÃO confundir com SIGA RJ (GENERIC_STRUTS_ACTION) sem evidência de que o RJ usa AZ Informática."
	},
	{
		...
		functional_family: "PROCUREMENT_TRANSACTIONAL",
		pncp_overlap: false,
		notes: "FATO — Transferegov: LICITAÇÕES-e / BANCO DO BRASIL S/A. Portal transacional e overlap PNCP PENDENTES de mapeamento nesta sessão."
	}
];
var BLL_SOURCES = [
	"src_bll_platform",
...
async function seedMilestone3(sql) {
	for (const jur of JURISDICTIONS) await upsert(sql, "jurisdiction", {
		...jur,
		cnpj: null,
		active: true
	});
	for (const sig of LEARNED_VENDOR_SIGNATURES) await upsert(sql, "fingerprint_signature", {
		id: sig.id,
	...
	await upsert(sql, "source_evidence", {
		...
		description: "Relação oficial dos sistemas eletrônicos de compras integrados ao Transferegov.br: LANCE ELETRÔNICO | BOLSA DE LICITAÇÕES E LEILÕES DO BRASIL | 10.508.843/0002-38.",
		url: TRANSFEREGOV,
	...
	await upsert(sql, "source_evidence", {
		...
		description: "Município de Cambará/PR publica a sessão na plataforma BLL, endereço https://bllcompras.com/Home/Login.",
	...
	await upsert(sql, "source_evidence", {
		...
		description: "Plataforma eletrônica: Bolsa de Licitações e Leilões do Brasil – BLL. Edital em https://bll.org.br/. Cita integração ao PNCP.",
		...
		vendor_name: "Bolsa de Licitações e Leilões do Brasil",
		product_name: "BLL Compras",
		raw_excerpt: "PLATAFORMA ELETRÔNICA PARA REALIZAÇÃO DO CERTAME-BOLSA DE LICITAÇÕES E LEILÕES DO BRASIL – BLL. EDITAL DISPONIVEL EM: https://bll.org.br/"
	});
	await upsert(sql, "source_evidence", {
		...
		description: "Prefeitura Municipal de Ubaíra realiza o certame na Bolsa de Licitações e Leilões do Brasil, https://bllcompras.com/Home/Login.",
		...
		vendor_name: "Bolsa de Licitações e Leilões do Brasil",
		product_name: "BLL Compras",
		raw_excerpt: "LOCAL: Bolsa de Licitações e Leilões do Brasil - https://bllcompras.com/Home/Login."
	});
	await upsert(sql, "source_evidence", {
		...
		description: "Prefeitura do Município de Cotia realiza o pregão na plataforma da BLL — Bolsa de Licitações e Leilões do Brasil Ltda.",
		...
		vendor_name: "Bolsa de Licitações e Leilões do Brasil",
		product_name: "BLL Compras",
		raw_excerpt: "CADASTRO DE PROPOSTAS INICIAIS E DISPUTA: http://www.bll.org.br — plataforma da BLL - Bolsa de Licitações e Leilões do Brasil Ltda."
	});
	await upsert(sql, "source_evidence", {
		...
		description: "GET https://bllcompras.com/Process/ProcessSearchPublic?param1=5 — HTTP 200, title Busca de Processos - BLLCOMPRAS, Server Microsoft-IIS/10.0, X-AspNetMvc-Version 5.2. Colunas: Promotor, Número, Modalidade, Cidade, Situação. Rotas /Process/ProcessView. Header ASP.NET é framework, não vendor.",
	...
	await upsert(sql, "source_evidence", {
		...
		description: "PORTAL DE COMPRAS PÚBLICAS | ECUSTOMIZE CONSULTORIA EM SOFTWARE LTDA | 09.397.355/0001-30.",
		url: TRANSFEREGOV,
	...
	await upsert(sql, "source_evidence", {
		...
		description: "Contratação do sistema Portal de Compras Públicas. Contratada: ECUSTOMIZE CONSULTORIA EM SOFTWARE S.A, CNPJ 09.397.355/0001-30.",
	...
	await upsert(sql, "source_evidence", {
		...
		description: "Prefeitura Municipal de Alegre declara adesão ao Portal de Compras Públicas, mantido pela Ecustomize Consultoria em Software S/A.",
	...
	await upsert(sql, "source_evidence", {
		...
		description: "Sistema SCPI – Portal de Compras – Pregão Eletrônico – FIORILLI, http://transparencia.itapira.sp.gov.br:8079/comprasedital/.",
	...
	await upsert(sql, "source_evidence", {
		...
		description: "SCPI – Portal de Compras – Pregão Eletrônico FIORILLI em http://170.0.49.246:5656/comprasedital/.",
		...
		vendor_name: "Fiorilli Software",
		product_name: "SCPI Portal de Compras",
		raw_excerpt: "SCPI – Portal de Compras – Pregão Eletrônico FIORILLI em http://170.0.49.246:5656/comprasedital/."
	});
	await upsert(sql, "source_evidence", {
		...
		description: "Diário oficial: Sistema Eletrônico SCPI – PORTAL DE COMPRAS (FIORILLI) em http://transparencia.urupes.sp.gov.br:5656/comprasedital/.",
		...
		vendor_name: "Fiorilli Software",
		product_name: "SCPI Portal de Compras",
		raw_excerpt: "Sistema Eletrônico SCPI – PORTAL DE COMPRAS (FIORILLI) http://transparencia.urupes.sp.gov.br:5656/comprasedital/"
	});
	await upsert(sql, "source_evidence", {
		...
		description: "Prefeitura de Assis publica a sessão no Portal de Compras Fiorilli https://scpi.assis.sp.gov.br:8079/comprasedital/.",
		...
		vendor_name: "Fiorilli Software",
		product_name: "SCPI Portal de Compras",
		raw_excerpt: "Local de Realização da Sessão Pública Portal de Compras Fiorilli https://scpi.assis.sp.gov.br:8079/comprasedital/"
	});
	await upsert(sql, "source_evidence", {
		...
		description: "Prefeitura Municipal de Sales/SP nomeia SCPI – Portal de Compras – Fiorilli em http://170.0.48.169:8079/comprasedital/.",
		...
		vendor_name: "Fiorilli Software",
		product_name: "SCPI Portal de Compras",
		raw_excerpt: "ENDEREÇO ELETRÔNICO: (http://170.0.48.169:8079/comprasedital/) - SCPI - Portal de Compras – Fiorilli. PREFEITURA MUNICIPAL DE SALES/SP."
	});
	await upsert(sql, "source_evidence", {
		...
		description: "wbc.pmf.sc.gov.br, domínio da Prefeitura de Florianópolis, identifica o sistema como 'Paradigma EGOV - florianopolis'. Título de produto no portal do ente, não inferência visual.",
	...
	await upsert(sql, "source_evidence", {
		...
		description: "compras.barueri.sp.gov.br hospeda manual de cadastro WBC (janeiro/2025) com marca www.paradigmabs.com.br.",
	...
	await upsert(sql, "source_evidence", {
		...
		description: "Relação oficial: e-LIC | SECRETARIA DE ESTADO DA ADM DE SANTA CATARINA | 82.951.351/0001-42. Sistema atual integrado NÃO é Paradigma. Histórico de imprensa 2014 permanece em source_platform_history.",
		url: TRANSFEREGOV,
		...
		vendor_name: null,
		product_name: "e-LIC",
		raw_excerpt: "e-LIC | SECRETARIA DE ESTADO DA ADM DE SANTA CATARINA | 82.951.351/0001-42"
	});
	await upsert(sql, "source_evidence", {
		...
		description: "Reportagem Baguete/ABES (2014) cita os estados de Pernambuco, Pará e Santa Catarina como usuários da solução WBC Public. NÃO é contrato vigente. NÃO sobrescreve a classificação atual GENERIC_JSON_API / e-LIC.",
		...
		raw_excerpt: "Outras cidades paulistas que também usam esta solução são Piracicaba, Sorocaba e Diadema, assim como municípios de outros estados como Florianópolis e sistemas federativos da indústria no RS, SC, PR e RJ, bem como os estados de Pernambuco, Pará e Santa Catarina."
	});
	for (const [id, source, title, cnpj] of [
		[
			"ev_transferegov_licitanet",
			"src_licitanet_platform",
			"LICITANET",
			"21.280.462/0001-80"
		],
		[
		...
		[
			"ev_transferegov_bbmnet",
			"src_bbmnet_platform",
			"BBMNET Licitações",
			"05.342.088/0001-43"
		],
		[
			"ev_transferegov_licitar",
			"src_licitar_platform",
			"LICITAR DIGITAL",
			"35.125.567/0001-79"
		],
		[
			"ev_transferegov_comprasbr",
			"src_comprasbr_platform",
			"PREGÃO ELETRÔNICO SIGA / COMPRASBR",
			"24.598.492/0001-27"
		],
		[
			"ev_transferegov_licitacoese",
			"src_licitacoese_platform",
			"LICITAÇÕES-e",
			"BANCO DO BRASIL S/A"
		]
	]) await upsert(sql, "source_evidence", {
	...
	for (const source of SOURCES) await upsert(sql, "source_platform_history", {
	...
	await upsert(sql, "source_alert", {
		...
		severity: "info",
		message: "Família BLL: 1 adapter generic-action + 8 configs. Adapter comercial BllAdapter = NO-GO — o genérico cobre o contrato HTML público sem forks por município.",
		payload: {
			family: "BLL",
	...
	await upsert(sql, "source_alert", {
		...
		severity: "info",
		message: "Família SCPI/Fiorilli: 5 entes VERIFIED com path /comprasedital. Adapter FiorilliAdapter = NO-GO — contrato HTML ao vivo não exercitado; não promover framework a fornecedor.",
		payload: {
	...
	await upsert(sql, "source_alert", {
		...
		alert_type: "SOURCE_FAMILY_CONFIRMED",
		severity: "info",
		message: "Paradigma/WBC: Florianópolis e Barueri VERIFIED vigentes. SC estadual é histórico (2014–2023) e atual e-LIC. ParadigmaAdapter = NO-GO.",
		payload: {
