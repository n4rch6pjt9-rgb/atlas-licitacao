import { a as leadTimeHours, c as recordOverlap, i as integrationPriorityScore, n as classifyLocalOnly, o as nextRecheckAt, r as compareProcurementFields, s as percentile } from "./coverage-h_nbs-Bf.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/coverage.server-DMEnGzqD.js
...
function excerptFromPayload(payload) {
	return readStr(asRecord(payload), [
		"objeto",
		"objetoCompra",
		"object",
		"numero",
...
function candidateFromRecord(row) {
	const payload = asRecord(row.raw_payload);
	const orgao = asRecord(payload.orgaoEntidade);
	return {
		canonical_entity_type: (row.technology_family ?? "") === "COMPRAS_GOV" || row.source_system_id === "src_br_comprasgov" ? "COMPRAS_GOV" : "PNCP",
		...
		organization_cnpj: readStr(payload, [
			"cnpj",
			"organizationCnpj",
			"orgaoCnpj"
		]) ?? readStr(orgao, ["cnpj"]),
		organization_identifier: readStr(payload, ["cnpj", "organizationCnpj"]) ?? readStr(orgao, ["cnpj"]),
		organization_name: readStr(payload, [
			"orgaoNome",
			"organization_name",
			"razaoSocial"
		]) ?? readStr(orgao, ["razaoSocial"]),
		year: Number(readStr(payload, ["ano", "year"]) ?? NaN) || null,
		sequential: readStr(payload, [
			"sequencial",
			"sequencialCompra",
			"sequential"
		]),
		process_number: readStr(payload, [
			"numero",
			"numeroCompra",
			"processo",
			"process_number"
		]),
		procurement_number: readStr(payload, [
			"numeroCompra",
			"numero",
			"procurement_number"
		]),
		modality: readStr(payload, [
			"modalidade",
			"tipo",
			"modality"
		]),
		object: readStr(payload, [
			"objeto",
			"objetoCompra",
			"object"
		]),
		opening_at: readStr(payload, [
			"dataAbertura",
			"abertura",
			"opening_at"
		]),
...
async function loadNationalCandidates(sql) {
	return (await sql.query(`select r.source_identifier, r.source_system_id, r.raw_payload, s.technology_family
     from source_record r
     join source_system s on s.id = r.source_system_id
     where s.technology_family in ('PNCP', 'COMPRAS_GOV')
        or r.source_system_id in ('src_br_pncp', 'src_br_comprasgov')`)).map(candidateFromRecord);
}
async function insertSourceEntityLink(sql, input) {
...
async function upsertCanonical(sql, id, source, extras) {
...
async function persistResolvedRecord(sql, source, candidates, meta) {
	...
	await sql.query(`update source_record
	...
	if (matched && resolved.canonical_entity_id) {
		...
		const [national] = await sql.query(`select first_seen_at from source_record
       where source_identifier = $1
         and source_system_id in ('src_br_pncp', 'src_br_comprasgov')
       order by first_seen_at asc
       limit 1`, [resolved.canonical_entity_id]);
		...
		if (winner) {
			const conflicts = compareProcurementFields(source.source_system_id, {
			...
			for (const conflict of conflicts) await sql.query(`insert into source_comparison (
		...
		canonicalId = `can_local_${source.source_system_id}_${source.external_id}`.replace(/[^a-zA-Z0-9_]/g, "_");
		await upsertCanonical(sql, canonicalId, source, {
			municipality: source.organization_name,
			uf: meta?.uf ?? null,
			ibge: meta?.ibge ?? null
...
async function resolveIngestedRecords(sql, sourceId, records) {
	if (records.length === 0) return;
	if (sourceId === "src_br_pncp" || sourceId === "src_br_comprasgov") return;
	const [source] = await sql.query(`select s.technology_family, j.uf as jurisdiction_uf, j.ibge_code
     from source_system s
     left join jurisdiction j on j.id = s.jurisdiction_id
     where s.id = $1`, [sourceId]);
	if (source?.technology_family === "PNCP" || source?.technology_family === "COMPRAS_GOV") return;
	const candidates = await loadNationalCandidates(sql);
	const stored = await sql.query(`select source_identifier, first_seen_at from source_record
     where source_system_id = $1`, [sourceId]);
	const firstSeenById = new Map(stored.map((row) => [row.source_identifier, iso(row.first_seen_at) ?? (/* @__PURE__ */ new Date()).toISOString()]));
	for (const record of records) await persistResolvedRecord(sql, record, candidates, {
		firstSeenAt: firstSeenById.get(record.external_id) ?? /* @__PURE__ */ new Date(),
		uf: source?.jurisdiction_uf ?? null,
		ibge: source?.ibge_code ?? null
	});
}
async function listMunicipalityCensus(sql) {
	return jsonSafe((await sql.query(`select c.*, s.name as source_name
     from municipality_census c
     left join source_system s on s.id = c.source_system_id
     order by c.uf nulls last, c.name`)).map((row) => ({
		id: String(row.id),
...
async function listLocalOnly(sql) {
	return jsonSafe((await sql.query(`select r.id, r.source_system_id, s.name as source_name, r.source_identifier,
...
async function listComparisons(sql) {
	return jsonSafe((await sql.query(`select c.*, p.object
...
async function listLiveProbes(sql) {
	return jsonSafe((await sql.query(`select p.*, s.name as source_name
     ...
     order by p.probed_at desc`)).map((row) => ({
		id: String(row.id),
		source_system_id: row.source_system_id == null ? null : String(row.source_system_id),
		source_name: row.source_name == null ? null : String(row.source_name),
		url: String(row.url ?? ""),
...
async function listSourceRecords(sql, sourceId) {
	return jsonSafe((await sql.query(`select r.*, l.status as match_status, l.match_method, l.match_score,
     ...
     limit 200`, [sourceId])).map((row) => {
		...
		return {
			...
			matched_fields: Array.isArray(matchedFields) ? matchedFields.map((item) => String(item)) : [],
...
async function searchProcurements(sql, filters = {}) {
	const clauses = [];
	const params = [];
	if (filters.q?.trim()) {
		params.push(`%${filters.q.trim().toLowerCase()}%`);
		const idx = params.length;
		clauses.push(`(lower(coalesce(c.object, '')) like $${idx}
	...
	return jsonSafe((await sql.query(`select c.*,
            (
     ...
     limit 200`, params)).map((row) => ({
		...
		sources: String(row.sources ?? "").split(" · ").map((item) => item.trim()).filter(Boolean),
		match_status: row.match_status == null ? null : String(row.match_status),
		local_only_state: row.local_only_state == null ? null : String(row.local_only_state),
		lead_time_hours: row.lead_time_hours == null ? null : num(row.lead_time_hours),
		source_url: row.source_url == null ? null : String(row.source_url),
...
async function milestone4Metrics(sql) {
	const [localCounts, linkCounts, localOnlyCounts, leadRows, adapters, censusCounts, declared, degraded, canonical, conflicts, familyRows, sources, probes] = await Promise.all([
		sql.query(`select coalesce(s.technology_family, 'UNKNOWN') as family, count(*)::int as n
       from source_record r
       join source_system s on s.id = r.source_system_id
       ...
       join source_system s on s.id = r.source_system_id
       where s.technology_family not in ('PNCP', 'COMPRAS_GOV')`),
		sql.query(`select lead_time_hours as hours, source_system_id
       from source_entity_link
       where lead_time_hours is not null`),
		sql.query(`select coalesce(nullif(connector_type, ''), 'NONE') as adapter, count(*)::int as count
       from source_system
       ...
       where classification_state in ('DEGRADED', 'DISABLED')`),
		sql.query(`select count(*)::int as n from canonical_procurement`),
		sql.query(`select count(*)::int as n from source_comparison`),
		sql.query(`select
	...
	const overlap = recordOverlap({
		...
		local_only_records: num(localOnlyCounts[0]?.pending) + num(localOnlyCounts[0]?.provisional) + num(localOnlyCounts[0]?.confirmed),
	...
	const positiveLeadSources = new Set(leadRows.filter((row) => num(row.hours) > 0).map((row) => row.source_system_id));
	const candidates = buildFamilyCandidates(sources);
	const reuseReady = sources.filter((row) => row.connector_type === "GENERIC_JSON" || row.connector_type === "GENERIC_ACTION" || row.connector_type === "GENERIC_JSF").length;
	const adapterReuse = sources.length === 0 ? 0 : reuseReady / sources.length;
	const municipalVerified = sources.filter((row) => (row.jurisdiction_type === "MUNICIPALITY" || row.jurisdiction_type === "PUBLIC_ENTITY") && row.vendor_evidence_level === "VERIFIED").length;
	const families5 = candidates.filter((row) => row.verified_sources >= 5).length;
	const declaredRatio = num(declared[0]?.total) === 0 ? 0 : num(declared[0]?.n) / num(declared[0]?.total);
	const leadByFamily = /* @__PURE__ */ new Map();
	...
	const family_priorities = familyRows.map((row) => {
		...
		const score = integrationPriorityScore({
			...
			technical_reuse_probability: generic === "GO" ? 1 : publicQ > .5 ? .35 : .1,
			public_access_quality: publicQ,
			adapter_reuse: generic === "GO" ? 1 : row.adapter ? .4 : 0,
			historical_depth: .2
		});
		const rationale = generic === "GO" ? "Generic reuse comprovado. Prioridade = ganho de records, não de catálogo." : commercial === "NO-GO" ? "Sem adapter comercial. Contrato público insuficiente ou escala baixa." : "Censo de entes ainda raso.";
		return {
			family: row.family,
			...
			public_access_quality: Math.round(publicQ * 1e3) / 1e3,
			priority_score: score,
			rationale
		};
	}).sort((a, b) => b.priority_score - a.priority_score);
	const bllEntes = sources.filter((row) => row.technology_family === "BLL" && row.id !== "src_bll_platform").length;
	const chosen = candidates.find((row) => row.technology_family === "BLL");
	const go = bllEntes >= 20 && bllRecords >= 100 && pncpMatched > 0 && num(localOnlyCounts[0]?.provisional) + num(localOnlyCounts[0]?.confirmed) > 0 && leads.length > 0 && num(canonical[0]?.n) > 0;
	return jsonSafe({
		...
		adapter_reuse_ratio: Math.round(adapterReuse * 1e3) / 1e3,
...
//#endregion
export { listComparisons, listLocalOnly, listMunicipalityCensus, listSourceRecords, milestone4Metrics, resolveIngestedRecords, searchProcurements };
