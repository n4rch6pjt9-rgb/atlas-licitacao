import { n as classifyLocalOnly } from "./coverage-h_nbs-Bf.mjs";
...
import { a as redactKey, i as fingerprintKey, n as classifyAuthError, r as classifyPublicKey, t as PCP_APIPCP_PUBLICKEY_EVIDENCE } from "./pcp-semantics-Y8Z50bdG.mjs";
import { r as schemaFingerprint } from "./schema-fingerprint-49fkWjyh.mjs";
import { f as upsertSourceCapability, o as insertSourceRecord } from "./queries.server-B9mr7TW7.mjs";
import { n as PRIORITY_WEIGHTS_V2, persistPrioritySnapshot } from "./priority.server-B4IJ691O.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/seed-m6.server-CDsUyzF1.js
async function upsertCheckpoint(sql, input) {
	const id = `ckpt_${input.source_system_id}_${input.window_start ?? "open"}`;
	await sql.query(`insert into ingestion_checkpoint (
...
var OBSERVED = "2026-09-14T22:55:00.000Z";
var WINDOW_ID = "win_2026_09_08_14_live";
var WINDOW_START = "2026-09-08T00:00:00.000Z";
var WINDOW_END = "2026-09-14T23:59:59.000Z";
var PCP_V2 = "https://compras.api.portaldecompraspublicas.com.br";
var PCP_SITE = "https://www.portaldecompraspublicas.com.br";
var PCP_APIPCP = "https://apipcp.portaldecompraspublicas.com.br";
var DISCOVER = "/v2/licitacao/processos";
var PER_SOURCE_LIMIT = 25;
var PAGE_SIZE = 25;
var LIVE_BUDGET_MS = 45e3;
...
async function configureLiveJson(sql, sourceId, defaultParams, acceptEquals) {
...
async function ingestLiveSource(sql, sourceId, defaultParams, acceptEquals) {
	const t0 = Date.now();
	try {
		...
		const records = [];
		for await (const record of adapter.discover({ limit: PER_SOURCE_LIMIT })) records.push(record);
       ...
       where source_system_id = $1
         ...
         and local_only_state is null`, [sourceId, classifyLocalOnly({
			matched: false,
...
async function seedMilestone6(sql) {
	const verdict = classifyPublicKey(PCP_APIPCP_PUBLICKEY_EVIDENCE);
	await upsertReplace(sql, "public_key_assessment", {
		...
		excerpt: "apidoc: publicKey = 'Chave de verificação', obrigatória. Doc: integração com sistemas internos; acesso ao ambiente de testes pode ser solicitado a comprador@portaldecompraspublicas.com.br. SPA /processos não embute publicKey. GET sem chave e com amostra documental: HTTP 400 Erro autenticação.",
		key_fingerprint: fingerprintKey("b7ad651a44cff961330fe393543796f0"),
		http_status: 400,
	...
	await upsertReplace(sql, "public_key_assessment", {
		...
		excerpt: "OpenAPI público sem security. GET /v2/licitacao/processos não exige publicKey. Filtro orgao é nome público do comprador. Esta é a API da SPA.",
		key_fingerprint: null,
		http_status: 200,
		observed_at: OBSERVED,
		notes: "Discovery público = compras.api v2 + config orgao. publicKey da apipcp permanece AUTH_REQUIRED."
	}, "id");
	await sql.query(`update source_record
	...
	for (const ente of LIVE_ENTES) {
		...
		if (Date.now() - liveStarted > LIVE_BUDGET_MS) {
			...
			await sql.query(`update source_connector_config set connector_readiness = 'DEGRADED' where source_system_id = $1`, [ente.id]);
			continue;
		}
		await sleep(180);
		const result = await ingestLiveSource(sql, ente.id, params, ente.acceptEquals);
		let schema = null;
		if (result.count > 0) schema = (await sql.query(`select schema_hash from source_record
         where source_system_id = $1 and ingestion_mode = 'LIVE_PUBLIC_API'
         limit 1`, [ente.id]))[0]?.schema_hash ?? null;
		liveResults.push({
			id: ente.id,
			label: ente.label,
			count: result.count,
		...
		await upsertReplace(sql, "source_probe_run", {
			...
			ok: result.count > 0,
			excerpt: result.error ? result.error.slice(0, 280) : `LIVE ${result.count} records · generic-json · orgao público`,
			error: result.error,
			fingerprint_excerpt: "compras.api v2 público; sem publicKey",
			probed_at: (/* @__PURE__ */ new Date()).toISOString()
		}, "id");
	}
	await configureLiveJson(sql, "src_pcp_platform", { limitePagina: String(PAGE_SIZE) });
	if (Date.now() - liveStarted > LIVE_BUDGET_MS) {
		liveResults.push({
			id: "src_pcp_platform",
		...
		await sql.query(`update source_connector_config set connector_readiness = 'DEGRADED' where source_system_id = 'src_pcp_platform'`);
	} else {
		await sleep(180);
		const platform = await ingestLiveSource(sql, "src_pcp_platform", { limitePagina: String(PAGE_SIZE) });
		liveResults.push({
			id: "src_pcp_platform",
			label: "Plataforma nacional",
	...
	try {
		const page1 = await publicGet(`${PCP_V2}${DISCOVER}?orgao=${encodeURIComponent("Prefeitura Municipal de Mogi das Cruzes")}&pagina=1&limitePagina=5`);
		await sleep(180);
		const page2 = await publicGet(`${PCP_V2}${DISCOVER}?orgao=${encodeURIComponent("Prefeitura Municipal de Mogi das Cruzes")}&pagina=2&limitePagina=5`);
		const ids1 = (page1.json?.result ?? []).map((row) => row.codigoLicitacao).filter(Boolean);
		const ids2 = (page2.json?.result ?? []).map((row) => row.codigoLicitacao).filter(Boolean);
		page2Distinct = ids1.length > 0 && ids2.length > 0 && ids1.every((id) => !ids2.includes(id));
		const first = ids1[0];
		if (first) {
			await sleep(180);
			itemsOk = (await publicGet(`${PCP_V2}/v2/licitacao/${first}/itens?pagina=1`)).ok;
			await sleep(180);
			...
			await sleep(180);
			detailOk = (await publicGet(`${PCP_V2}/v2/licitacao/sp/prefeitura-municipal-de-mogi-das-cruzes-1352/cptp-15-2026-2026-507402`)).ok;
		}
		await upsertReplace(sql, "source_probe_run", {
			...
			ok: page2Distinct,
			excerpt: page2Distinct ? `página 2 ≠ página 1 (${ids1[0]} vs ${ids2[0]}). PAGE_NUMBER.` : "paginação não comprovada nesta sessão",
			error: null,
	...
	for (const ente of [...LIVE_ENTES, { id: "src_pcp_platform" }]) for (const cap of capRows) await upsertSourceCapability(sql, {
		...
		documented: true,
		confidence: cap.observed ? .9 : .2
	});
	const liveCount = await sql.query(`select count(*)::int as n
	...
	const hashes = [...new Set(liveResults.map((row) => row.schema).filter(Boolean))];
	...
	await upsertReplace(sql, "coverage_window", {
		...
		metric_version: "v2",
		notes: "Coorte LIVE do M6. Não reutiliza a coorte de avaliação M5. Overlap e lead medidos sobre records LIVE_PUBLIC_API."
	}, "id");
	await sql.query(`insert into family_coverage_metric (
     ...
     on conflict (window_id, family) do nothing`, [WINDOW_ID]);
	...
	await sql.query(`update family_coverage_metric
	...
	const go = verdict.semantics === "PRIVATE_CREDENTIAL" && !verdict.allowedInPublicConnectorConfig && liveEntes >= 5 && liveN >= 100 && page2Distinct && ranking.length >= 5;
	await upsertReplace(sql, "operational_report", {
		id: "opr_m6_pcp",
		family: "PORTAL_COMPRAS_PUBLICAS",
		window_id: WINDOW_ID,
		score_version: "v2",
		payload: {
			...
			pcp_vs_bll: {
				...
				coverage_gain_per_engineering_day: "INFERÊNCIA — PCP v2 público reusa generic-json já existente; BLL já ingerido no M4. Ganho residual PCP > BLL enquanto o censo nacional PCP não estiver capturado."
			},
	...
	await sql.query(`update source_system
     set notes = $1
     where id = 'src_pcp_platform'`, [`M6 FATO — publicKey apipcp = PRIVATE_CREDENTIAL / AUTH_REQUIRED. Discovery live = GET ${PCP_V2}${DISCOVER} sem chave, filtro orgao por ente. PcpAdapter NÃO criado. ${liveN} records LIVE.`]);
}
LIVE_ENTES.map((row) => row.id);
//#endregion
export { seedMilestone6 };
