import { i as pncpEditalUrl, r as pncpControl } from "./pncp-url-CkM9bQPG.mjs";
...
import { m as normalizeObject } from "./recurrence-Dg8bgN42.mjs";
import { r as resolveOrgIdentity, t as arpSignals } from "./org-identity-DpYJSdnO.mjs";
import { runIntelligenceJobs } from "./intelligence.server-BvA1TlYE.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/seed-m7.server-CqzYmzzC.js
var NOW = "2026-09-14T20:00:00.000Z";
var pncpSeq = 1001;
var JSONB = /* @__PURE__ */ new Set([
	"raw_payload",
	"payload",
	"matched_fields",
	"conflicting_fields",
...
async function upsert(sql, table, row, pk = "id") {
	...
	const placeholders = keys.map((key, i) => JSONB.has(key) ? `$${i + 1}::jsonb` : `$${i + 1}`).join(", ");
...
async function insertProcurement(sql, seed) {
	...
	const control = seed.cnpj ? pncpControl({
		cnpj: seed.cnpj,
		unidade: 1,
		numero: pncpSeq++,
	...
	const localOnly = seed.localOnly ? classifyLocalOnly({
		matched: false,
		firstSeenAt: seed.firstLocal,
		now: NOW
	}) : matched ? "MATCHED" : classifyLocalOnly({
		matched: false,
		firstSeenAt: seed.firstLocal,
		now: NOW
	});
	...
	if (matched && seed.firstPncp) {
       ...
       on conflict do nothing`, [
			...
			payloadHash({
				...payload,
				numeroControlePNCP: control ?? `pncp-${ext}`
			}),
			JSON.stringify({
				...payload,
				numeroControlePNCP: control ?? `pncp-${ext}`
...
async function seedMilestone7(sql) {
	pncpSeq = 1001;
	for (const cat of [
		CAT_SPORTS,
		CAT_FOOD,
	...
	for (let i = 0; i < sportDates.length; i += 1) {
		await insertProcurement(sql, {
			...
			firstPncp: new Date(new Date(sportDates[i]).getTime() + 1296e5).toISOString()
		});
		await upsert(sql, "price_observation", {
			...
			unit_price: 18e4 + i * 5e3,
			total_price: 18e4 + i * 5e3,
	...
	await sql.query(`insert into opportunity_event (
     ...
     on conflict (id) do nothing`, [
		...
		JSON.stringify({
			...
			to: "2026-09-20T17:00:00.000Z"
		}),
		"ev_opp_sport_meta",
		JSON.stringify({ field: "notes" })
	]);
	await generateChangeAlerts(sql);
	for (const [id, key, type, entity, notes] of [
		[
			"golden_early_matched",
			"early_matched",
			"opportunity",
			"opp_can_m7_sport_now",
			"Local 10/09, PNCP 13/09"
		],
		[
			"golden_local_only_prov",
			"local_only_provisional",
			"opportunity",
			"opp_can_m7_local_only",
			"Sem PNCP, dentro da janela"
		],
		[
			"golden_local_only_conf",
			"local_only_confirmed",
			"source_record",
			"existing_pcp_cohort",
			"Coorte PCP/BLL já materializada"
		],
		[
			"golden_pca_converted",
			"pca_converted",
			"planning_record",
			"plan_pca_porto_sport",
			"CNPJ+ano+CATMAT"
		],
		[
			"golden_pca_unmatched",
			"pca_unmatched",
			"planning_record",
			"plan_pca_palmas_veic",
			"Sem contratação"
		],
		[
			"golden_pca_probable",
			"pca_probable",
			"planning_record",
			"plan_pca_mogi_merenda",
			"Objeto+valor, sem id oficial"
		],
		[
			"golden_pgc_converted",
			"pgc_converted",
			"planning_record",
			"plan_pgc_office",
			"PGC ≠ PCA"
		...
		[
			"golden_non_recurring",
			"non_recurring_purchase",
			"canonical_procurement",
			"can_m7_pave",
			"1 compra"
		],
		[
...
async function generateChangeAlerts(sql) {
	const { generateAlertEvents } = await import("./intelligence.server-BvA1TlYE.mjs");
	await generateAlertEvents(sql);
}
//#endregion
export { seedMilestone7 };
